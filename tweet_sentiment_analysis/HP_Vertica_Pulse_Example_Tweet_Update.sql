/** Save the following SQL code in .sql file somewhere on your local file system
* You set a python scheduler to run this code so that the dt_start and dt_end tables are dropped and recreated with the correct information on which data has been analyzed
*/
...
INSERT INTO tweet_sentiment(SELECT id, "user.screen_name", SentimentAnalysis(text USING PARAMETERS filterlinks = true, filterusermentions = true) OVER(PARTITION BY id, "user.screen_name", text) FROM tweets WHERE lang = 'en' 
